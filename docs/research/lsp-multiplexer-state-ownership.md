...
下表“基线状态”指 0.1.0 squash 前的工作快照；该快照没有作为 Git ref 进入发布
历史，实现后的决议见上节。
...
[`eglotx--tag-owned-object`](../../eglotx.el)、
[`eglotx--restore-owned-object`](../../eglotx.el) 和
[`eglotx--transform-client-params`](../../eglotx.el)。新 module 应吸收这些
...
当前实现只接受 `workspace/didChangeWatchedFiles` dynamic registration，并按 backend
保存 child registration identity；semantic/notebook 与 dynamic workspace file-operation
registration 被拒绝。Initialize-time static registration IDs 被剥离而不是虚拟化，
workspace-folder `changeNotifications` string 则归一化为 boolean support。实现见
[`eglotx--handle-registration-request`](../../eglotx.el)。
...
性能可预测的降级。
[`eglotx--transform-client-progress-tokens`](../../eglotx.el)
...
上述规则已经覆盖 document 与 related-document diagnostics；cursor 还绑定 exact
open-document object，close/reopen 后即使 URI 相同也会强制 full。当前 facade 不暴露
...
[`eglotx--merge-semantic-capability`](../../eglotx.el)
